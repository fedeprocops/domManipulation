//START PROGRAMMA
function ShowInput(nome, cognome, localita_presa, data, genere) {
    //APPUNTI
    ...
    // ARRAY
    const consonanti_nome = [];
    const vocali_nome = [];
    const consonanti_cognome = [];
    const vocali_cognome = [];
    // VARIABILI FILTRO
    let consonanti;
    let vocali;
    // VARIABILE CONTROLLO
    let consonanti_prese_nome;
    let consonanti_prese_cognome;
    //CONTATORI
    let n_cons = 0; // CONTA CONS
    ///####################ANNO DATI##############################///
    let anno = data.getFullYear().toString().substr(-2);
    ///####################MESE E GIORNO##############################///
    let mese = data.getMonth() + 1; //perché getmonth parte da 0
    ...
    if (mese >= 1 && mese <= 12) {
      mese = cod_mese[mese - 1];
    }
  
    if (giorno >= 1 && giorno <= 31 && genere == "F") {
      giorno += 40;
    }
  
    giorno = String(giorno).padStart(2, 0);
    ...
    const CF_FX = GetCF(CF); // TOGLIE LE VIRGOLE SEPARANO LE CONSONANTI DEI NOMI E COGNOMI
  
    //#SOMMA FINALE
    const somma_controllo = ControlSum(CF_FX);
  
    return CF_FX + caratteri_lista[somma_controllo];
  }
  
  function ControlSum(CF_FX) {
    const alpha_dispari = [];
    const alpha_pari = [];
    ...
    return (somma_dispari + somma_pari) % 26;
  }
  ...
  const cod_mese = ["A", "B", "C", "D", "G", "H", "L", "M", "P", "R", "S", "T"];
  ...
  const province = new Map([
    ...
    ["CURINO", "D219"],
    ["CURNO", "D221"],
    ["CURON VENOSTA .GRAUN IN VINSCHGAU.", "D222"],
    ["CURSI", "D223"],
    ["CURSOLO-ORASSO", "D225"],
    ["CURTAROLO", "D226"],
    ["CURTATONE", "D227"],
    ...
    ["SANTA CRISTINA VALGARDENA .ST CHRISTINA IN G.", "I173"],
    ...
    ["TROVO", "L453"],
    ["TRUCCAZZANO", "L454"],
    ["TUBRE .TAUFERS IN MUNSTERTHAL.", "L455"],
    ["TUENNO", "L457"],
    ["TUFARA", "L458"],
    ["TUFILLO", "L459"],
    ["TUFINO", "L460"],
